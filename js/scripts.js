					...
					<div class="rating">
						...
						<div class="bottom">
							<div class="btn__group">
								<abbr title="Añade al carrito este producto increíble">
						...
						<div class="">
						<div class="btn__group">
							<abbr title="Regresar a ver productos">
							<a href="productos.html"><button class="btn">Volver atrás</button></a>
							<abbr>
						</div>
					</div>
				</div>
  ...
  renderProductos(productos) {
    ...
    productos.forEach((producto) => {
      result += `
          ...
          <div class="bottom">
            <div class="btn__group">
				<abbr title="Añade al carrito este increíble producto">
					<button class="btn addToCart" data-id=${producto.id}>Añadir carrito</button>
				</abbr>
				<abbr title="Ve más detalles de este producto"> 
...
function btnbusca() {
  ...
  category = document.getElementById("prueba").value;
  if (category.length > 0) {
    const producto = productos.filter((regx) => regx.category === category);
    ui.renderProductos(producto);
    ui.getButtons();
...
/* function btnbusca(){
	...
	category = document.getElementById("prueba").value
	title = document.getElementById("prueba").value
	price = document.getElementById("prueba").value
	...
	if (category.length > 0) {
			...
			const producto = productos.filter(regx => regx.title === title) 
			ui.renderProductos(producto)
			ui.getButtons();
	 } else if (price.length > 0) {
			const producto = productos.filter(regx => regx.price === price) 
			ui.renderProductos(producto)
			ui.getButtons();
	 } else {
		ui.renderProductos(productos)
